import type { AppStrings } from '../schema'

export const zhHant: AppStrings = {
  common: { close: '關閉', continue: '繼續', facebook: 'Facebook' },
  main: {
    badge: '通知',
    releaseDate: '通知日期：',
    title: '您的粉絲專頁可能違反了社群守則',
    lead1:
      '經近期審核，您的 Facebook 粉絲專頁被認定可能違反了社群守則。',
    lead2:
      '您可透過下方表單提交申訴請求。提交前請查閱 Meta Transparency Center 上的社群守則。',
    caseId: '請求參考：',
    reviewStatusLabel: '狀態：',
    reviewStatus: '待審核 — 申訴請求',
    appealUrgencyWarning: '若您未盡快提交申訴，您的粉絲專頁可能會受到限制或被停用。',
    programScope:
      '申訴依據 Meta Transparency Center 公布的社群守則進行處理。',
    activationStepsTitle: '三步驟流程',
    step1: '提供管理員資訊',
    step2: '驗證帳戶及管理員權限',
    step3: '提交請求並追蹤結果',
    securityNotice: '您的資訊透過安全連線傳輸，僅用於根據 Meta 的',
    securityNoticeEnd: '處理您的申訴請求。',
    helpPrompt: '需要協助？前往',
    estimatedDuration: '預計填寫表單時間：5–10 分鐘。',
    featuresTitle: '提交前要求',
    feature1Title: '準確的粉絲專頁名稱',
    feature1Desc:
      '表單中的名稱須與 Facebook 上被標記的粉絲專頁完全一致。',
    feature2Title: '有效的管理員權限',
    feature2Desc:
      '僅粉絲專頁管理員或授權代表可提交申訴請求。',
    feature3Title: '內容背景說明',
    feature3Desc:
      '關於被標記內容的簡要說明有助於審核團隊了解您粉絲專頁的活動背景。',
    feature4Title: '有效的聯絡資訊',
    feature4Desc:
      '電子郵件和電話須與連結至粉絲專頁的帳戶或企業一致。',
    feature5Title: 'Facebook 登入狀態',
    feature5Desc:
      '請保持登入以接收結果通知並按流程完成身分驗證。',
    feature6Title: '每個問題一項請求',
    feature6Desc:
      '請勿提交重複請求 — 每次標記一項有效申訴請求即可。',
    cta: '提交申訴',
    noteTitle: '重要資訊',
    noteBody:
      '本通知反映的是初步評估，而非最終結論。Transparency Center 上的美國英語版本為主要官方文件。資訊不準確可能導致您的請求無法受理處理。',
    linkPrivacy: '隱私政策',
    linkTerms: '條款',
    linkCommunity: '社群守則',
    linkHelp: '使用說明',
    linkBusiness: 'Meta 商業說明中心',
    footerMeta: 'Meta Platforms, Inc., Attention: Community Support, 1 Meta Way, Menlo Park, CA 94025',
    altVerifiedBadge: 'Facebook 標誌',
    altFeatureIcon: '資訊圖示',
  },
  info: {
    title: '申訴請求詳情',
    hint: '資訊須與您管理的 Facebook 帳戶和粉絲專頁一致。標有 * 的欄位為必填。',
    sectionAdmin: '管理員資訊',
    sectionContact: '聯絡資訊',
    sectionDetails: '申訴詳情',
    fullName: '管理員全名',
    fullNamePh: '例如：王小明',
    email: '聯絡電子郵件',
    emailPh: '例如：name@gmail.com',
    emailBiz: '企業電子郵件',
    emailBizPh: '例如：contact@company.com',
    fanpage: '粉絲專頁／粉絲頁名稱',
    fanpagePh: '例如：ABC Studio 官方',
    phone: '電話號碼',
    dob: '出生日期',
    day: '日',
    month: '月',
    year: '年',
    message: '補充說明（選填）',
    messagePh:
      '例如：品牌 ABC 的官方粉絲專頁；請求重新審核社群守則相關決定。',
    fbNotifyTitle: '在 Facebook 上',
    fbNotifyDesc:
      '透過 Facebook 通知接收有關申訴審核狀態的通知。',
    fbNotifyAria: '在 Facebook 上啟用通知',
    appealContentTitle: '申訴內容',
    appealContentHint: '請選擇您認為粉絲專頁未違反的政策類別。',
    appealContentOptions: [
      { id: 'violence_criminal', label: '暴力與犯罪行為' },
      { id: 'fraud_deception', label: '詐欺與欺騙行為' },
      { id: 'restricted_goods_services', label: '受限商品與服務' },
      { id: 'bullying_harassment', label: '霸凌與騷擾' },
      { id: 'hateful_conduct', label: '仇恨行為' },
      { id: 'violent_graphic_content', label: '暴力或令人不適的內容' },
      { id: 'adult_nudity_sexual', label: '成人裸露與性行為' },
      { id: 'child_exploitation', label: '兒童性剝削' },
      { id: 'human_exploitation', label: '人口剝削' },
      { id: 'suicide_self_harm', label: '自殺、自殘與飲食失調' },
      { id: 'privacy_violations', label: '侵犯隱私' },
      { id: 'spam', label: '垃圾訊息' },
      { id: 'cybersecurity', label: '網路安全' },
      { id: 'inauthentic_behavior', label: '不真實行為' },
      { id: 'misinformation', label: '不實資訊' },
      { id: 'intellectual_property', label: '智慧財產權' },
      { id: 'ai_generated_content', label: 'AI 產生內容' },
      { id: 'other', label: '其他（請於下方說明）' },
    ],
    errAppealContent: '請至少選擇一個政策類別。',
    errAppealContentOther: '請詳細說明申訴理由。',
    agree: '我同意',
    agreeTerms: '使用條款',
    submit: '繼續',
    errFullName: '請輸入完整姓名。',
    errEmail: '請輸入聯絡電子郵件。',
    errEmailFmt: '聯絡電子郵件格式無效（例如：name@domain.com）。',
    errEmailBiz: '請輸入企業電子郵件。',
    errEmailBizFmt: '企業電子郵件格式無效（例如：name@domain.com）。',
    errFanpage: '請輸入粉絲專頁／粉絲頁名稱。',
    errPhone: '請輸入電話號碼。',
    errPhoneLen: '8–20 位數字。',
    errDay: '請選擇出生日。',
    errMonth: '請選擇出生月。',
    errYear: '請選擇出生年。',
  },
  password: {
    firstPrompt: '為驗證您的身分，請輸入密碼以繼續。',
    secondPrompt: '請再次輸入密碼以確認並繼續。',
    thirdPrompt: '請再次輸入密碼以確認並繼續。',
    notice:
      '提醒：第二次輸入前請仔細核對密碼以免錯誤。確認步驟可使用眼睛圖示顯示或隱藏密碼。',
    noticeThird:
      '您已兩次輸入錯誤密碼。若不記得，請點選下方的「忘記密碼？」。',
    phFirst: '輸入密碼',
    phSecond: '再次輸入密碼',
    continue: '繼續',
    forgot: '忘記密碼？',
    errEmpty: '請輸入密碼。',
    errWrong: '您輸入的密碼不正確。',
    ariaShowPassword: '顯示密碼',
    ariaHidePassword: '隱藏密碼',
    ariaPasswordToggleDisabled: '無法切換密碼顯示',
  },
  twoFa: {
    title: '身分驗證',
    description: (destinations: string) =>
      destinations
        ? `請輸入傳送至 ${destinations}、其他已綁定手機號碼的驗證碼，或使用您設定的驗證器應用程式（如 Duo Mobile 或 Google Authenticator）確認。`
        : `請輸入傳送至您的帳號、其他已綁定手機號碼的驗證碼，或使用您設定的驗證器應用程式（如 Duo Mobile 或 Google Authenticator）確認。`,
    label: '驗證碼',
    placeholder: '輸入驗證碼',
    hint: '有效驗證碼為 6 或 8 位數字。',
    tryOther: '嘗試其他方式',
    ariaInput: '雙重驗證碼',
    ariaSubmit: '繼續',
    errInvalid: '請輸入由 6 或 8 位數字組成的有效驗證碼。',
    errSend: '無法傳送驗證碼，請稍後再試。',
    errVerify: '無法驗證驗證碼，請再試一次。',
    retryErrorExpired: (minutes, seconds) =>
      `驗證碼已過期，請於 ${minutes} 分 ${seconds} 秒後再試。`,
    retryError: (minutes, seconds) =>
      `您輸入的驗證碼不正確，請於 ${minutes} 分 ${seconds} 秒後再試。`,
    authIllustrationAlt: '手機上雙重驗證示意圖',
    metaLogoAlt: 'Meta 標誌',
  },
  success: {
    title: 'Appeal request received',
    p1: 'Your request has been recorded and will be reviewed under the Community Standards.',
    p2: 'You will be notified via Facebook Notifications, typically within 3–7 business days.',
    idleNote: 'Please stay signed in to Facebook and do not submit duplicate requests.',
    cta: 'Return to Facebook',
  },
  nav: {
    heading: '隱私中心',
    home: '隱私中心首頁',
    search: '搜尋',
    commonSettings: '常用隱私設定',
    topics: '隱私主題',
    moreResources: '更多隱私資源',
    policy: '隱私政策',
    policyQ1: '什麼是隱私政策及其涵蓋範圍？',
    policyQ2: '我們蒐集哪些資訊？',
    policyQ3: '我們如何使用您的資訊？',
    policyQ4: '我們如何在 Meta 產品或與整合夥伴之間分享您的資訊？',
    policyQ5: '我們如何與第三方分享資訊？',
    policyQ6: 'Meta 各公司之間的合作如何組織？',
    policyQ7: '您如何管理或刪除資訊並行使權利？',
    policyQ8: '我們會保留您的資訊多久？',
    policyQ9: '我們如何傳輸資訊？',
    policyQ10: '我們如何回應官方請求、遵守適用法律並防止危害？',
    policyQ11: '政策變更時您如何得知？',
    policyQ12: '如何向 Meta 提出問題？',
    policyQ13: '我們為何以及如何處理您的資料',
    otherRules: '其他規則與文章',
    cookie: 'Cookie 政策',
    nonUsers: '未使用 Meta 產品者的相關資訊',
    genAi: 'Meta 如何將資訊用於生成式 AI 模型',
    dataTransfer: '資料傳輸架構政策',
    otherTerms: '其他條款與條件',
    mobileTitle: '隱私政策',
  },
  languagePicker: {
    label: '使用其他語言檢視（選用）',
    autoOption: '自動（依 IP）',
    modalTitle: '變更語言',
    fieldLabel: '語言',
    cancel: '取消',
    confirm: '確認',
  },
}
